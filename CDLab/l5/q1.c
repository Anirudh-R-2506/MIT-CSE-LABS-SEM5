...
typedef struct localTables
{
    int id;
    struct symbol* table[10];
} localTables;

symbol* global_table[10];
localTables* tables[10];
...
int charBelongsTo(int c, char t)
{
    ...
    if (t == 'a'){
        ...
        len = (sizeof(specialsymbols) - 2) / sizeof(char); 
        arr = specialsymbols;
        if (c == '}'){
            if (gotOpenBracket != 1)
                gotOpenBracket--;
